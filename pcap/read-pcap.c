...
//  const struct icmphdr *my_icmp;
//  
//  my_icmp = (const struct icmphdr *) packet;
...
void analyze_icmp2(const u_char *packet){ // see /usr/include/netinet/ip_icmp.h
...
void analyze_ip(const u_char *packet){    // see /usr/include/netinet/ip.h
  ...
  case IPPROTO_ICMP:   // ICMP protocol = 1
    ...
    analyze_icmp2(packet + header_len);          // move the pointer to the beginning of ICMP header
	break;
